...
- `albertogarob/whistlerlib:dev` image present locally (the test fixture builds it on first run; ~5–10 min the first time for R + radvertools).
...
The Compose stack validates exactly the same image and network story that production Swarm uses, same `daskdev/dask` scheduler image, same `albertogarob/whistlerlib` image, same TCP wire protocol. **Production deployment is Swarm via `docker/stack.yml`**; that path is the one operators use with `docker stack deploy` on a real multi-node cluster.
