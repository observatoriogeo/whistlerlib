...
For the cluster itself (scheduler + workers), see [Docker installation](/docs/installation/docker).

## Requirements

- **Python ≥ 3.11**.
- A reachable Dask scheduler (a local one via `LocalCluster`, or a remote one, see [Architecture](/docs/concepts/architecture)). The pip install does **not** start a cluster for you.
...
Instead, the R libraries live **inside the published `albertogarob/whistlerlib` Docker image** ([Docker installation](/docs/installation/docker)). The pattern is:
...
- [Docker installation](/docs/installation/docker), bring up a local cluster (Compose) or production cluster (Swarm).
- [Tutorial 01](/docs/tutorials/01-quickstart-hashtag-histogram), first end-to-end run.
