...
flowchart LR
    ...
    subgraph WORKERS["Workers (one process per Docker container)"]
        W1["Worker 1<br/>albertogarob/whistlerlib"]
        W2["Worker 2"]
        WN["Worker N"]
    end
...
All four base primitives, `compute_vector_histogram`, `compute_vector_range`, `compute_matrix_nz_histogram_and_sum`, `compute_weighted_coonet`, follow this "distributed map_partitions → distributed groupby/agg → local sort" shape. See [Algorithm families](/docs/concepts/algorithm-families) for which user-facing method funnels into which primitive.
