...
If you've never used the 0.1.0 codebase, skip this page, `pip install whistlerlib` is your starting point, see [Install from PyPI](/docs/installation/pip).
...
5. **Deploy via Docker** if you used the legacy `docker/linode/` setup, the production target is now Docker Swarm or Compose (see [Install with Docker](/docs/installation/docker)).
...
- **Single custom image**: `albertogarob/whistlerlib:<version>`. Carries whistlerlib + Dask + R + the full R library set. Both worker and scheduler roles in the published Docker Compose / Swarm stacks use this image; the scheduler service overrides the entrypoint to `dask-scheduler`.
...
See [Install with Docker](/docs/installation/docker) for the full deployment story.
