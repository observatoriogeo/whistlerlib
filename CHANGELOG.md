...
- **`albertogarob/whistlerlib` Docker image**: built with `uv` in a multi-stage `python:3.11-bookworm` base, shipping the whistlerlib package + R + the full R library set (`r-cran-tm`, `r-cran-slam`, `r-cran-snowballc`, `r-cran-reshape2`, `r-cran-dplyr`, `r-cran-tidyr`, `r-cran-nlp`, `r-cran-stringr`, `r-cran-remotes`, `r-cran-vctrs`, `r-cran-arrow`) + `RWeka` / `syuzhet` / `arrow` from Posit Package Manager + `radvertools` from upstream GitHub. This is the only custom image whistlerlib publishes; both the scheduler ("master") and worker services in `docker/docker-compose.yml` and `docker/stack.yml` use this image, with the master service overriding the `ENTRYPOINT` to `dask-scheduler`.
