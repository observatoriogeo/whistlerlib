...
import pytest
from .conftest import P, combs_p_r, validate_hashtag, validate_graph
...
def test_not_null_graph(p, tweet_dataset):
    ...
    * Check that the number of nodes of the resulting graph is > 0
    ...
    _, graph = tweet_dataset.hashtag_weighted_coonet()
...
def test_not_empty_df(p, tweet_dataset):
    ...
    * Check that the number of rows of the resulting DataFrame is > 0
    ...
    graph_df, _ = tweet_dataset.hashtag_weighted_coonet()
...
def test_p1_eq_plt1_graph(p, tweet_dataset, tweet_dataset_p1):
    ...
    * Check that the degree lists of the two runs are identical
    ...
    _, graph_plt1 = tweet_dataset.hashtag_weighted_coonet()
    _, graph_p1 = tweet_dataset_p1.hashtag_weighted_coonet()
...
def test_p1_eq_plt1_df(p, tweet_dataset, tweet_dataset_p1):
    ...
    * Check that the graph DFs of the two runs are identical 
    ...
    graph_df_plt1, _ = tweet_dataset.hashtag_weighted_coonet()
    graph_df_p1, _ = tweet_dataset_p1.hashtag_weighted_coonet()
...
def test_reps_graph(p, r, tweet_dataset):
    ...
    * Check that the degree lists of the two runs are identical
    ...
    _, graph = tweet_dataset.hashtag_weighted_coonet()
    _, graph_rep = tweet_dataset.hashtag_weighted_coonet()
...
def test_reps_df(p, r, tweet_dataset):
    ...
    * Check that the graph DFs of the two runs are identical  
    ...
    graph_df, _ = tweet_dataset.hashtag_weighted_coonet()
    graph_df_rep, _ = tweet_dataset.hashtag_weighted_coonet()
...
@pytest.mark.parametrize('p', P)
@pytest.mark.usefixtures('tweet_dataset')
def test_validate_hashtags(p, tweet_dataset):
    ''' Scenario:
    * Repartition the dataset if needed
    * Check that the partitioning was successful
    * Run the whistlerlib method with a given p
    * Check that the hashtags in the graph's nodes are valid
    * Check that the hashtags in the DataFrame's source and target columns are valid    
    '''
    ...
    graph_df, graph = tweet_dataset.hashtag_weighted_coonet(
        return_time_profile=False)

    for node in graph.vs:
        node_hashtag = node['name']
        validate_hashtag(node_hashtag)

    for _, row in graph_df.iterrows():
        source_hashtag = row['source']
        target_hashtag = row['target']
        validate_hashtag(source_hashtag)
        validate_hashtag(target_hashtag)
...
def test_validate_graph(p, tweet_dataset):
    ...
    * Validate the returned graph against the returned DataFrame    
    ...
    graph_df, graph = tweet_dataset.hashtag_weighted_coonet(return_time_profile=False)
